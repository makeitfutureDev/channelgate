...
test("the landing lock ref carries the product name", async (t) => {
  const cwd = await scratchRepository(t);
  assert.equal(resolveLandingLock(cwd).ref, "refs/channelgate/landing-lock");
  const lock = await acquireLandingLock({ cwd, owner: "ref-name", hostname: "test-host" });
  const refs = execFileSync("git", ["-C", cwd, "for-each-ref", "--format=%(refname)", "refs/channelgate/"]).toString().trim();
  assert.equal(refs, "refs/channelgate/landing-lock");
  assert.equal(
    execFileSync("git", ["-C", cwd, "for-each-ref", "refs/claude-gateway/"]).toString().trim(),
    "",
    "nothing is written under the pre-rename name",
  );
  assert.equal(await releaseLandingLock(lock), true);
});
