...
for (const variant of ["previous", "root", "none"]) {
  test(`current controls bypass the engine with ${variant} historical attachments`, async () => {
    for (const [command, expected] of [["/files", /The \/files command has been removed/], ["stop", /Nothing is running/], ["pending", /follow|pending|waiting/i], ["/help", /ChannelGate|Commands|commands/], ["/next", /Add the task after/]]) {
