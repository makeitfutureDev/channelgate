...
test("a message arriving in a busy thread asks whether to steer or queue", async () => {
  ...
  let choiceId = "";
  const processing = processMessageEvent(event, client, { botUserId: "U_BOT", teamId: "T_BUSY" });
  try {
    for (let attempt = 0; attempt < 100 && client.posted.length === 0; attempt++) await delay(10);
    const choice = client.posted.find((message) => Array.isArray(message.blocks));
    assert.ok(choice, `expected a choice card, got ${JSON.stringify(client.posted)}`);
    assert.equal(choice.thread_ts, THREAD);
    const buttons = choice.blocks.flatMap((block) => block.elements || (block.accessory ? [block.accessory] : []));
    assert.deepEqual(buttons.map((button) => button.text?.text), ["Steer Conversation", "Add to Queue", "Cancel Request"]);
    choiceId = buttons[0]?.value || "";
  } finally {
    if (choiceId) busyThreadChoices.discard(choiceId);
    runQueue.abort(runKey);
...
test("Add to Queue leaves the active run untouched and executes after it releases", async () => {
  ...
  const queue = card?.blocks.flatMap((block) => block.elements || []).find((button) => button.action_id === BUSY_THREAD_QUEUE_ACTION);
  assert.ok(queue);
