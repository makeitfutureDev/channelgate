...
test("writable Codex runs use the gateway-workspace permission profile with no runtime-root grant", () => {
  ...
  assert.ok(args.includes(`permissions.gateway-workspace.filesystem={ ":root" = "deny", ":minimal" = "read", ${TOOLCHAIN_READS}, ":tmpdir" = "write", ":slash_tmp" = "deny", ":workspace_roots" = { "." = "write", ".git" = "read", ".codex" = "read" } }`));
...
test("read-only Codex runs use the gateway-readonly permission profile with no temp write grant", () => {
  ...
  assert.ok(args.includes(`permissions.gateway-readonly.filesystem={ ":root" = "deny", ":minimal" = "read", ${TOOLCHAIN_READS}, ":tmpdir" = "deny", ":slash_tmp" = "deny", ":workspace_roots" = { "." = "read" } }`));
  assert.ok(args.includes(`features.network_proxy.enabled=false`));
