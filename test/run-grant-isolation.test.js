...
import { access, lstat, mkdir, mkdtemp, readFile, readdir, readlink, rm, stat, symlink, writeFile } from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { ensureTestEnv } from "./helpers.js";

const gatewayRoot = ensureTestEnv();
const { assertUserSkillOverlaySupported, createRunGrantArtifacts } = await import("../src/gateway/run-grant-artifacts.js");
...
test("engine state overlays preserve auth/session only and omit host-global customizations", async (t) => {
  ...
  t.after(() => artifacts.cleanup());
  assert.equal(await readlink(path.join(artifacts.claudeConfigDir, ".credentials.json")), path.join(claudeState, ".credentials.json"));
