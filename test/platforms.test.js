...
import {
  ...
  livePlatformIds, platformOr, isPlatformId,
...
test("the built-in platforms are registered with Slack as the default and only live surface", () => {
  assert.deepEqual(PLATFORM_IDS, ["slack", "googlechat", "msteams"]);
  assert.equal(DEFAULT_PLATFORM, "slack");
  assert.deepEqual(livePlatformIds(), ["slack"]);
});
