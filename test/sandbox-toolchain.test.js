...
const [{ toolchainReadPaths, TOOLCHAIN_BINS }, { buildSettings }, { saveSettings, getCliIntegrations }, { gatewayRoot }] =
  await Promise.all([
...
function fakeHome() {
  const home = mkdtempSync(path.join(os.tmpdir(), "cg-toolchain-"));
...
function fakeHomeSplitGlobals() {
  const home = mkdtempSync(path.join(os.tmpdir(), "cg-toolchain-split-"));
  const bin = path.join(home, ".local", "bin");
  const prefixBin = path.join(home, ".local", "node", "bin");
...
test("paths outside HOME are dropped — only the read-denied tree needs re-allowing", () => {
  const fx = fakeHome();
  const elsewhere = mkdtempSync(path.join(os.tmpdir(), "cg-usrbin-"));
...
test("a missing binary is skipped silently — no throw, no phantom path", () => {
  const empty = mkdtempSync(path.join(os.tmpdir(), "cg-empty-"));
  const home = mkdtempSync(path.join(os.tmpdir(), "cg-barehome-"));
