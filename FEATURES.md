...
- Background AGENTS (`run_agent_in_background`) — the durable subagent: the daemon runs a separate
  ...
  streams its progress into the job tail, then re-invokes the launching thread with the agent's
  report (capped 12k chars) exactly like shell background jobs. Allowed in every channel mode
  ...
  "interrupted"). → TEST-PLAN: Background jobs.
...
- Native Slack **channel file explorer**: `/files` opens a Block Kit modal rooted at the channel's
  ...
  *Browse channel files* message shortcut opens it for a selected thread, typed `@bot /files` posts
  ...
  named directory in the folder on screen. When a public URL is configured, the single *Upload files / folder* button
...
- Busy-thread steer-or-queue choice: a message sent while the thread is mid-turn pauses behind an
  ...
  buttons instead of steering automatically. Cancel drops only the paused message (durably, so no
  restart or stale click revives it) and deliberately leaves the run in progress going — stopping
  that is what `stop` / 🛑 is for. Steering interrupts warm Claude in-protocol and cleanly terminates a cold
  Claude or Codex child before the selected message runs next; queuing leaves the active run untouched
  and preserves FIFO order. Outstanding cards survive daemon restarts; acceptance atomically converts
  the pending card into a recoverable active run, while the abandoned target is terminalized before
  its interrupt so restart cannot replay both turns. `stop` invalidates unresolved cards and aborts
  engines before waiting on Slack notices; another author's steer selection queues rather than
  interrupting their work. Expired/replayed/foreign clicks cannot duplicate or hijack the message.
  A card is only ever raised for a message the gateway is NOT already handling: Slack redelivers
  unacked envelopes (and a restart wipes both in-memory dedupes), so a copy of a message whose turn
  is already active, queued, or awaiting its own card is dropped silently instead of asking the user
  about something they never re-sent — and boot recovery skips a row a redelivered copy already
  picked up, so the same message is never answered twice.
  Prefix **`/next`** to queue directly without the extra choice.
  → TEST-PLAN: In-thread commands.
...
- Slack **Lists** (the native table/tracker): the gateway control MCP exposes `slack_list_create`,
  ...
  pasted List URL. Available to any allowed user (no admin gate). → TEST-PLAN: Slack Lists.
...
- **Toolchain reachability inside the sandbox** (`src/gateway/toolchain-paths.js`): the lockdown
  ...
  admin runs now re-allow READING the resolved toolchain: the Node install prefix (npm, npx and
  ...
  (plant-a-binary-now, run-it-unsandboxed-later) is untouched. → TEST-PLAN: CLI integrations.
...
- The Admin UI API reference includes the verified Make.com `slack:CreateMessage` blueprint for
  triggering Atlas through Slack: live Atlas/Make IDs, saved explicit mention, approved bot-user and
...
- **Linux hosts keep their Bash sandbox under Ubuntu's AppArmor userns restriction**: Ubuntu 23.10+
  ...
  restriction. → TEST-PLAN: Security (Linux userns sandbox).
...
- **Users** is a table (Name / Slack ID / role chips / C·S·T token state) with a
  ...
  Save — unchanged PUT semantics); "+ Add user" reveals the add form.
  → TEST-PLAN: Admin UI (redesign).
...
- Settings page: a vertical section nav — **Connection** (Slack bot tokens + status + the daemon's
  public URL), **Agent defaults** (enabled harnesses, engine, cross-engine failover, keepalive, context window, progress
  view, mention reactions, instructions, memory, schedule limits, nudges, codex rate),
  **Integrations** (Composio/Skills/Toolbox URLs + org-default tokens), **Access & security**
  (default channel access, trusted bot apps + network domains as chip editors), **System** (admin
  password, Reconnect Slack, and a red **danger zone**: reset all channels' access, remove
  password, restart daemon, disconnect Slack). All sections stay in the DOM and persist through ONE
  Save, presented as a sticky save bar with dirty tracking ("Unsaved changes" → "All changes
  saved"); Slack reconnects only when a Slack token actually changed. Destructive actions use
  branded in-app confirm dialogs (danger-tinted confirm, Escape cancels) instead of native
  `confirm()`/`alert()`. → TEST-PLAN: Admin UI (redesign).
...
- **Boot migration `scripts/migrate-channelgate.mjs`**: runs once at boot — before the database
  ...
  audit of all of it that exits 1 while anything still points at a pre-rename root; `--dry-run` ends
  with the same audit. → TEST-PLAN: ChannelGate rename. A refusal or failure never fails the
