...
import { stableClaudeState } from "./run-grant-artifacts.js";
...
export async function runMemoryReview({ client = null, channelId, slug, threadKey, authorId, meta = {}, fetchTranscript = null, reason = "" }) {
  ...
  // the gateway root as a normal run (never the shared tmpdir: see run.js).
  const mcpConfigJson = await buildMcpConfig({
    ...
    gatewayFsRoot: allowedFsRoot(), gatewayWorkspaceRoot: workspaceRoot(), toolset: REVIEW_TOOLSET,
  });
  const mcpConfigFile = path.join(runTmpDir(), `cg-mcp-review-${randomUUID()}.json`);
  await mkdir(path.dirname(mcpConfigFile), { recursive: true, mode: 0o700 });
  await writeFile(mcpConfigFile, mcpConfigJson, { mode: 0o600 });
  const { claudeHome, claudeConfigDir } = await stableClaudeState();
  ...
  try {
    const result = await runClaude({
      ...
      strictMcp: true,
      settingsFile: channelSettingsFile(slug, meta.platform), // the channel's own lockdown — never the admin variant
      model,
