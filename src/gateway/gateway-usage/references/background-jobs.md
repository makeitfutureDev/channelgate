...
hand the work to the **gateway daemon**, which outlives your turn and re-invokes you when the
work finishes. Two daemon tools cover the two shapes of work:
...
folder and lockdown) on your task and re-invokes this thread with the agent's full report when it
finishes.
...
when the job finishes you are re-invoked in THIS thread with the result (a plain-language success,
failure, interruption, or time-limit outcome plus the output tail for shell; the agent's report for
agents) — your session resumes with full context, so you continue from where you left off
automatically. Raw process numbers are diagnostic metadata, never the explanation shown to users.
...
3. On re-invocation, read the result and continue (report outcome, next steps, etc.).
