...
4. **Delegate work that should outlive this turn to the daemon, not to a background subagent.**
   In-turn Agent/Task subagents are fine (a Stop hook mechanically prevents you from ending the
   turn while one is still running — if it blocks you, wait and collect the results). But anything
   meant to continue AFTER your reply must go to the gateway daemon: `run_in_background` for shell
   commands, `run_agent_in_background` for a delegated agent task. The daemon re-invokes this
   thread with the result when it finishes. Read `references/background-jobs.md` for the hand-off
   protocol.
