 ...
 */
export function renderContainerSshdConfig(dir) {
  return [
...
/** Plain `type base64 comment` lines — what the in-container sshd matches the second handshake against. */
export function renderContainerAuthorizedKeys(keys) {
  ...
  for (const key of keys) {
    ...
    lines.push(`${key.type} ${key.base64} cg:${key.userId}`);
  }
  return `${lines.join("\n")}\n`;
}
...
export function materializeContainerSshFiles(target, keys) {
  const dir = containerSshDir(target);
  mkdirSync(dir, { recursive: true, mode: 0o700 });
  chmodSync(dir, 0o700);
  writePrivate(path.join(dir, "sshd_config"), renderContainerSshdConfig(dir));
