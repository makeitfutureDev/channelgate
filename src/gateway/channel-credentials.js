...
function egressLines(names, { placeholders = {}, hosts = {}, unprotected = [], withheld = [], scopes = {} } = {}) {
  const lines = [];
...
export function channelCredentialsPreamble(resolved = {}, { clean = false, scopes = {}, placeholders = {}, hosts = {}, unprotected = [], withheld = [] } = {}) {
  if (clean) return "";
  ...
  return "[Channel credentials for THIS attempt]\n"
    ...
    + egressLines(names, { placeholders, hosts, unprotected, withheld, scopes }).map((line) => `${line}\n`).join("")
