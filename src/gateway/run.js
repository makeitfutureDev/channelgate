...
import { runTmpDir, workspaceRoot } from "../config/paths.js";
...
const INTERACTIVE_RUN_ORIGINS = new Set(["slack_foreground", "api_foreground"]);
...
export async function runMessage({ channelId, authorId, workspaceId = "", text, threadKey, attachments = [], signal = null, onDelta, onEvent, onRuntimeResolved, sessionId: presetSessionId = "", overrides = null, getFallbackContext = null, preferCold = false, progressReport = false, untrustedPrincipal = false, origin = "" }) {
  ...
  const { cwd, settingsFile, adminSettingsFile } = await ensureChannelFolder(entry.slug, provisionMeta, { runMeta: meta });
  ...
  let sessionId, isNew, sessionEngine = engine, switchedEngine = false;
  ...
  if (presetSessionId) {
    await saveSession(entry.slug, threadKey, presetSessionId, engine, sessionGen);
    sessionId = presetSessionId;
    isNew = true;
  } else {
    ({ sessionId, isNew, engine: sessionEngine } = await resolveSession(entry.slug, threadKey, engine));
    ...
    if (picked.switch) {
      ...
      sessionId = await resetSession(entry.slug, threadKey, engine, sessionGen);
      isNew = true;
      switchedEngine = true;
  ...
  const mintGatewayMcpRuntime = async () => {
    ({ mcpConfigJson, mcpConfigFingerprint, gatewayCapability } = await buildEngineMcpRuntime({ ...mcpRuntimeInput, engine }));
  };
  ...
  try { onRuntimeResolved?.({ engine, model }); } catch { /* non-fatal */ }
  ...
  const mcpConfigFile = usesMcpConfigFile(engine) ? path.join(runTmpDir(), `cg-mcp-${randomUUID()}.json`) : "";
  ...
  const runOnce = async (sid, fresh, promptOverride = null, modelOverride = model) => {
    ...
    return adapter.run(validateRunContext({
      ...
      runtime: {
        ...
        model: modelOverride, effort, permissionPromptTool, timeoutMs, maxSilenceMs, signal, onDelta, onEvent: scopedOnEvent,
  ...
  const runFallbackEngine = async (note, failureContext = "a usage/spend limit") => {
    ...
    const fallbackMcpRuntime = await buildEngineMcpRuntime({ ...mcpRuntimeInput, engine: fallbackEngine });
    ...
    try { onRuntimeResolved?.({ engine: fallbackEngine, model: fallbackModel }); } catch { /* non-fatal */ }
    ...
    const exec = async (fbSid, fbFresh, modelOverride = fallbackModel) => {
      ...
      return fallbackAdapter.run(validateRunContext({
        ...
        runtime: { preferCold: true, keepAliveMs: 0, poolKey: `${entry.slug}::${fbKey}`, dangerouslySkip,
          ...
          attachments, signal, timeoutMs, maxSilenceMs, onDelta, onEvent: scopedOnEvent,
    ...
    try {
      ...
      try { onRuntimeResolved?.({ engine: fallbackEngine, model: defaultModel }); } catch { /* non-fatal */ }
      try {
    ...
    if (cx.sessionId) await saveSession(entry.slug, fbKey, cx.sessionId, fallbackEngine, sessionGen);
  ...
  try {
    ...
    try {
      ...
      if (defaultModel) {
        ...
        try { onRuntimeResolved?.({ engine, model: defaultModel }); } catch { /* non-fatal */ }
        try {
        ...
        if (fallbackKind) {
          ...
          sid = await resetSession(entry.slug, threadKey, engine, sessionGen);
          fresh = true;
    ...
    if (mintsOwnSessionId(engine) && result.sessionId && result.sessionId !== sid) {
      await saveSession(entry.slug, threadKey, result.sessionId, engine, sessionGen);
    }
    ...
    if (sessionEngine === "" && !fresh && finalSessionId) {
      await saveSession(entry.slug, threadKey, finalSessionId, engine, sessionGen);
    }
    ...
    return {
      ...finalResult,
      content: licenseWarning + (finalResult.content || ""),
      loopWakeup,
