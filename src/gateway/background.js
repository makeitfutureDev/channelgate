...
export class BackgroundJobs {
  constructor({ slack, requestShellApproval, runner, deliver } = {}) {
    this.slack = slack;
    ...
    try {
      ...
      try {
        ...
        for (const j of this.jobs.values()) {
          ...
          ins.run(j.id, toJson({ id: j.id, kind: j.kind || "shell", channelId: j.channelId, slug: j.slug, authorId: j.authorId, threadKey: j.threadKey, label: j.label, command: j.command, task: j.task || "", startedAt: j.startedAt, pid: j.pid || null, startTime: j.startTime || "", maxMs: j.maxMs, logFile: j.logFile || "", approvalId: j.approvalId || "", pendingDelivery: j.pendingDelivery || null, deliveryAttempts: j.deliveryAttempts || 0 }));
        }
        db.exec("COMMIT");
      } catch (e) {
        db.exec("ROLLBACK");
  ...
  async _start({ channelId, slug, authorId, threadKey, command, label, maxMs = 0, kind = "shell", task }, approval = {}) {
      ...
      if (!approval.approvalGranted && !adminAuthorInAdminMode) {
        ...
        try {
          ...
          decision = await this.requestShellApproval({
            ...
            requiredTier: "admin", // unsandboxed daemon-account exec: only an admin's click counts
        ...
        if (!decision?.allow) {
          return {
            ...
            error:
              ...
              "Shell jobs run unsandboxed on the daemon, so Auto mode needs an explicit admin approval. " +
    ...
    const rec = { id, kind, channelId, slug: entry.slug, authorId, threadKey, label: name, command: cmd, task: taskText, startedAt, child: null, pid: null, maxMs, logFile, tail: "", timedOut: false, approvalId: approval.approvalId || "" };
    const onChunk = (chunk) => {
      try {
    ...
    if (isAgent) {
      ...
      let child;
      try {
        // Minimal allowlisted env — this IS agent-authored shell, so daemon secrets must not be visible.
        // detached → own process group, so the cap-kill takes any grandchildren too (see util/proc.js).
        child = spawn("bash", ["-lc", cmd], { cwd, env: buildChildEnv(), stdio: ["ignore", "pipe", "pipe"], detached: true });
      } catch (e) {
        logStream?.end();
  ...
  async _deliver(rec) {
    ...
    try {
      ...
      // The continuation is a real turn in a real Slack thread, so it takes the SAME per-thread
      // queue as a live message. Calling runMessage directly let it resume the thread's session
      // concurrently with whatever a human had just sent — two engines in one session, in one cwd.
  ...
  async _tailFromLog(logFile) {
    if (!logFile) return "";
    try {
      const txt = await readFile(logFile, "utf8");
      return txt.slice(-MAX_TAIL);
    } catch {
      return "";
    }
  }
  ...
  async _recoverJobs() {
    ...
    for (const rec of recs) {
      rec.tail = await this._tailFromLog(rec.logFile);
      if (rec.pendingDelivery) {
        ...
        await this._deliver(rec);
      } else if (pidIdentityAlive(rec.pid, rec.startTime)) {
        await logEvent("bg_recover_watch", { id: rec.id, slug: rec.slug, pid: rec.pid });
        this._watchRecovered(rec);
      } else {
  ...
  // Poll a recovered, still-running PID until it exits (we can't re-attach to its stdio), honoring
  // the remaining time cap, then continue the thread with the tail read from the log file.
  _watchRecovered(rec) {
    ...
    const tick = async () => {
      ...
      const alive = pidIdentityAlive(rec.pid, rec.startTime);
        ...
        await this._finish(rec, {
          outcome: rec.timedOut
            ...
            : {
                ok: null,
                kind: "restart_unknown",
                summary: "finished after the gateway restart, but its result could not be confirmed",
              },
        });
      }
    };
