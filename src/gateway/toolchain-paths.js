...
// Codex's permission-profile implementation may expose an individually granted symlink as a
// regular file. That breaks Node launchers such as npm: the copied npm-cli.js still executes, but
// its relative `../lib/cli.js` import is now resolved from ~/.local/bin instead of its package.
// Return only the reviewed HOME-local commands so the per-run grant builder can place direct
// symlinks to their real targets in one narrowly granted directory. Granting that directory keeps
// symlink semantics intact without exposing every unrelated file in the host's ~/.local/bin.
export function toolchainLauncherEntries({
  home = os.homedir(),
  root = null,
  integrations = [],
  dirs,
  ...
  for (const id of normalizeCliIntegrations(integrations)) bins.push(...(CLI_INTEGRATIONS[id].bins || []));
  const entries = [];
  for (const name of new Set(bins)) {
    const found = resolveBinPath(name, dirs);
    if (!found || !within(found, home) || (root && within(found, root))) continue;
    const target = realpathOr(found);
    if (!within(target, home) || (root && within(target, root))) continue;
