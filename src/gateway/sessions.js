...
function getRow(slug, threadKey) {
  return getDb().prepare("SELECT session_id, engine FROM sessions WHERE slug = ? AND thread_key = ?").get(slug, threadKey) || null;
}
...
function put(slug, threadKey, sessionId, engine = "") {
  getDb()
    .prepare("INSERT INTO sessions(slug, thread_key, session_id, engine) VALUES(?, ?, ?, ?) ON CONFLICT(slug, thread_key) DO UPDATE SET session_id = excluded.session_id, engine = excluded.engine")
    .run(slug, threadKey, sessionId, engine);
}
...
export async function resolveSession(slug, threadKey, engine = "") {
  const existing = getRow(slug, threadKey);
  if (existing && existing.session_id) return { sessionId: existing.session_id, isNew: false, engine: existing.engine || "" };
  const sessionId = randomUUID();
  put(slug, threadKey, sessionId, engine);
  return { sessionId, isNew: true, engine };
}
...
export async function saveSession(slug, threadKey, sessionId, engine = "", generation = null) {
  if (!sessionId) return;
...
export async function resetSession(slug, threadKey, engine = "", generation = null) {
  const sessionId = randomUUID();
  if (!generationStale(slug, threadKey, generation)) put(slug, threadKey, sessionId, engine);
  return sessionId;
}
...
export async function clearSession(slug, threadKey) {
  ...
  const info = getDb()
    .prepare("INSERT INTO sessions(slug, thread_key, session_id, engine) VALUES(?, ?, '', '') ON CONFLICT(slug, thread_key) DO UPDATE SET session_id = '', engine = ''")
    .run(slug, threadKey);
