...
export function subagentStopHooks() {
  return { Stop: [{ hooks: [{ type: "command", command: `node "${STOP_SUBAGENTS_HOOK}"` }] }] };
}
...
export async function buildSettings(meta, { allowBypass = false } = {}) {
  ...
  const filesystem = {
    denyRead: [sandboxPath(home), sandboxPath(root)],
  ...
  if (bashy || allowBypass) {
    ...
    filesystem.allowRead = [...new Set([...filesystem.allowRead, ...toolchain])];
  }
...
// work dir.
export async function ensureChannelFolder(slug, meta, { runMeta = meta } = {}) {
  ...
  if (meta.adminMode) {
    ...
    adminSettings.sandbox.enabled = false;
