...
import { resolveSlackConfig, getProgressView, getContextWindow, getEngine, getTrustedBotApps, getDefaultChannelAccess, getDefaultNudges, getSlackAdminUserToken, canChangeChannelRuntime, getWhisperEnabled } from "../config/settings.js";
...
export async function ensureRegistered(client, event) {
  ...
  const isNew = !meta;
  if (!meta) {
    meta = defaultChannelMeta({ channelId: event.channel, ...info });
  ...
  try {
    ...
    if (sc && !compactPassthrough) {
      ...
      if (sc.cmd === "help") {
        ...
        if (!sc.arg) {
          const cmd = buildResumeCommand(workDir, sessionId, threadEngine);
          await reply(
            cmd
    ...
    // A normal follow-up no longer chooses steering on the user's behalf. Hold the canonical
    // Slack event in a bounded, expiring durable store and ask its author what to do. The action
    // handler re-enters this pipeline with busyChoice="steer" or "queue". No persistent thread
    // setting changes and no attachment download happens until that choice is made.
