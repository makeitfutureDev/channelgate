...
export const HELP_TEXT =
  "*How to use me*\n" +
  "In a DM, just send your request. In a channel, write `@agent your request`; keep follow-ups in the thread and @mention me again there. For typed commands in a channel, use forms such as `@agent /help`. The registered `/files`, `/status`, and `/stop` Slack commands run at conversation top level without a mention.\n\n" +
  ...
  "• *Control a live run:* type `stop` in that thread or react 🛑 to stop just that run. `/stop` at top level stops every active run in this channel. If you send another message while I work, choose *Steer Conversation*, *Add to Queue*, or *Cancel Request* (drops just that message and leaves the run going); `/next <message>` queues directly without asking.\n" +
  "• *Open files:* attach a file/image and ask me to read it, or use `/files` to browse this channel's folder, create new files/folders, and upload multiple files or a folder directly through the secured browser flow when configured—without storing the upload in Slack. In Worker/Auto mode, valid UTF-8 text files, including `.env`, JSON, configs, and scripts, can be edited regardless of extension; protected managed/credential/key paths stay read-only. You can also preview and share a file into the conversation. From a thread, send `@agent /files`.\n" +
  ...
  "• *Add skills from the library:* DM me `set my Skills Manager token to …`, delete the token message, then star skills at skillsmanager.uk. Your favourites load automatically; say `use the <skill name> skill` when you want one explicitly.\n" +
  ...
  "• *Gateway skills:* `gateway-usage` is installed automatically and teaches me Slack tools, formatting, reminders, files, and administration. `channel-memory` is automatic when channel memory is enabled—there is nothing to install.\n" +
  ...
  "• `/stop` — stop every active run in this conversation (top-level; use `stop` or 🛑 for one thread)\n" +
