...
import { buildResumeCommand, resumeButton, filesButton, footerButtons, footerText, footerBlocks } from "./footer.js";
...
export { buildResumeCommand, resumeButton, filesButton, footerButtons, footerText, footerBlocks };
...
export async function fileExplorerContext(client, { channelId, userId, expectedSlug = "", verifyMembership = false } = {}) {
  const entry = await getChannelEntry(channelId);
  if (!entry || (expectedSlug && entry.slug !== expectedSlug)) throw new Error("This channel file explorer expired. Open it again with `/files`.");
  const meta = await getChannelMeta(entry.slug);
  ...
  if (!isAuthorized(meta, userId, Boolean(meta.isDM), { isAdminUser: userIsAdmin, isApprovedUser: userIsApproved })) {
    throw new Error("You're not authorized to browse files in this channel.");
  }
    ...
    try {
      if (event.user === botUserId) {
        ...
        const meta = await patchChannelMeta(entry.slug, (current) => {
          if (current) return {};
          const fresh = defaultChannelMeta({ channelId: event.channel, ...info });
