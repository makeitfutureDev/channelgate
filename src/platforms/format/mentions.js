...
// A name → id directory built from a plain roster, for platforms whose member list is not Slack's.
//
// The Slack builder (slack/directory.js) applies two anti-mistagging rules that are not optional:
// a name two different people answer to is DROPPED rather than guessed, and a single-word key that
// is several people's first name is dropped too (so "@Sam" resolves to nobody while "@Sam Lee"
// still resolves exactly). Silence beats pinging the wrong colleague. Those rules live here as well
// so a Chat space or a Teams roster inherits them instead of quietly re-introducing the bug.
//
// `people` is [{ id, names: [...] }]; `normalizeName` is the platform's key normalizer (the Slack
// one is already platform-neutral and is what every caller passes).
const MAX_DIRECTORY_WORDS = 5;

export function buildNameDirectory(people, normalizeName) {
  const map = new Map();
  ...
  return { map, maxWords: Math.min(maxWords, MAX_DIRECTORY_WORDS) };
}
