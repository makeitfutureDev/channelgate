...
export const USERNS_CODEX_STALE_HINT =
  "Codex approved-domain networking will fail (its network_proxy resets every tunnel, allowed " +
  "domains included) — re-run: sudo sh scripts/apparmor/claude-userns-fix.sh --apply to install " +
  "the updated profile with the codex-userns block (see scripts/apparmor/README.md)";
...
export function assessLinuxUserns({
  ...
  if (restricted && profileInstalled) {
    // A pre-Codex install of the profile exempts Claude's binaries only. Codex's sandbox is the
    // same userns architecture, and without attachment its approved-domain network_proxy loses
    // capabilities and resets every tunnel — a failure nothing in the turn output explains. The
    // installed file is world-readable, so a content sniff is enough to say "re-run --apply".
    const installed = readTrimmed(readFile, USERNS_PROFILE_PATH);
    if (installed !== null && !installed.includes("codex-userns")) {
      return {
        ...base,
        broken: true,
        reason: `${USERNS_PROFILE_PATH} is installed but predates Codex coverage (no codex-userns profile block)`,
        hint: USERNS_CODEX_STALE_HINT,
      };
    }
  }
