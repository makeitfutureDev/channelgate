...
export class PersistentClaudeSession {
  constructor({ cwd, args, env = null, idleMs = 10 * 60 * 1000 }) {
    this.cwd = cwd;
    this.args = args;
    this.env = env;
    this.idleMs = idleMs;
  ...
  start() {
    ...
    this.child = spawn("claude", this.args, { cwd: this.cwd, env: this.env || buildChildEnv(), stdio: ["pipe", "pipe", "pipe"], detached: true });
  ...
  _die(err) {
    ...
    killTree(this.child, "SIGKILL"); // group kill — takes the MCP children with it
    this.onDead?.();
  }
  ...
  send(text, { onDelta = null, onEvent = null, timeoutMs = 10 * 60 * 1000, maxSilenceMs = null } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const watchdog = createStallWatchdog({
        ...
        isAlive: () => pidAlive(this.child?.pid),
