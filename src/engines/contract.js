...
export const PRINCIPAL_KIND_BY_ORIGIN = Object.freeze({
  ...
  // A Google Chat / Teams turn is authored by a human the platform authenticated for us, exactly
  // like a Slack one. It is a separate origin rather than a shared "chat_foreground" so the audit
  // trail says which surface a run came from, and so escalation stays a per-surface decision
  // instead of one a new platform inherits by naming itself.
...
export function validateRunContext(context) {
  ...
  // Optional, but never half-supplied: `target` is the RuntimeTarget (src/runtimes/) the runner
  // spawns, probes and signals through. A target without a backend object would silently fall back
  // to the host — a container channel running unconfined on the daemon's filesystem is exactly the
  // failure this contract exists to make impossible.
  if (context.target !== undefined && context.target !== null && typeof context.target.runtime?.spawn !== "function") {
    throw new TypeError("RunContext.target must carry a runtime backend (target.runtime.spawn)");
  }
...
export function validateEngineAdapter(adapter) {
  ...
  // or the carry would silently no-op and the loss it exists to prevent would come back unseen.
  if (adapter.sessionState !== undefined) {
    const state = adapter.sessionState;
    if (!state || typeof state.hostDir !== "function" || typeof state.files !== "function" || typeof state.containerDirKey !== "string" || !state.containerDirKey) {
