...
function fingerprint({ cwd, mcpConfigJson, dangerouslySkip, fingerprintExtra }) {
  return `${cwd}|${dangerouslySkip ? 1 : 0}|${fingerprintExtra || ""}|${mcpConfigJson || ""}`;
}
...
export function createSessionPool({
  ...
  async function runPooled({ key, cwd, args, env, idleMs, mcpConfigJson, dangerouslySkip, fingerprintExtra, text, turnTimeoutMs, maxSilenceMs, signal, onDelta, onEvent }) {
    if (closed) throw shutdownError();
    const fp = fingerprint({ cwd, mcpConfigJson, dangerouslySkip, fingerprintExtra });
    ...
    if (!entry) {
      ...
      const session = createSession({ cwd, args, env, idleMs });
      session.onDead = () => {
