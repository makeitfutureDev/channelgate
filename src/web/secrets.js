...
const READERS = {
  settings: {
    ...
    // Google Chat's service-account key and the Teams client secret. Same terms as every other
    // credential here: never on a listing response, re-authenticated and audited on the way out.
    googleChatServiceAccountJson: (s) => s.googleChatServiceAccountJson,
    teamsAppPassword: (s) => s.teamsAppPassword,
    ...
    // The Claude subscription token container channels run on (`claude setup-token` on the gateway
