...
import { gatewayRoot, channelFolder } from "../config/paths.js";
...
export function createWebApp({
  ...
  app.use("/api", createAdminRouter({ slack, instanceId, startGatewayUpdate, restartCoordinator }));
  ...
  app.get("/resume", async (req, res) => {
    ...
    const resumeCmd = resumeCommandFor(getEngine(), session);
    // The gated folder is namespaced by the channel's platform; resolve it from the record rather
    // than assuming Slack, or the printed command would cd into a folder that does not exist.
    const platform = (await listChannels()).find((c) => c.slug === slug)?.meta?.platform;
    const cmd = `cd "${channelFolder(slug, platform)}" && ${resumeCmd}`;
