...
// Per-channel RUNTIME ARTIFACTS a containerized run must read at the SAME absolute path on both
// sides (the per-run --settings copy, the 0600 --mcp-config file, plugin dirs, Codex output and
// secret bundles). They live under the VISIBLE workspace root — beside the channel folders, never
// under the gateway root, which no container may mount — in a hidden sibling tree so the project
// folder itself stays clean. Dir 0700, files 0600. Host-backend runs keep using runTmpDir() and the
// metadata folder exactly as before.
export function runtimeArtifactsRoot() {
  return path.join(workspaceRoot(), ".runtime");
}
export function channelArtifactDir(slug, platform) {
  return path.join(runtimeArtifactsRoot(), platformFolder(platform), slug);
}
// The daemon-hosted MCP socket: bind-mounted read-only into every container at /run/channelgate so
// the gateway control MCP needs neither a DB mount nor a network path (P2, plan §11 addendum).
export function runtimeSocketDir() {
  return path.join(gatewayRoot(), "run");
}
...
export function installId() {
  ...
  try {
    ...
    /* root not created yet — the resolved path is still stable */
  }
