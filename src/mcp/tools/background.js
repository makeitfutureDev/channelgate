...
export function register(server, ctx) {
  ...
  // channel's folder and posts the self-contained agent report directly when it finishes.
  server.registerTool(
    "run_agent_in_background",
    {
      description:
        "Launch a BACKGROUND AGENT: the gateway daemon runs a separate engine session (Claude or Codex, " +
        "same channel folder/lockdown/mode) on the given TASK and POSTS ITS FINAL REPORT DIRECTLY into " +
        "this thread when it finishes. Use this INSTEAD of the " +
        ...
        "The report uses the normal sanitized/chunked reply pipeline and does not depend on a second " +
        "parent-model turn. Returns a job id; after that, STOP and end your turn.",
      ...
      try {
        ...
        return text(
          `✅ Started background agent *${data.label}* (id ${data.id}). It's running on the daemon now. ` +
            `END YOUR TURN — the daemon will post the agent's final report directly in this thread when it finishes. ` +
            `Do not wait or poll.`
        );
      } catch (e) {
  ...
  server.registerTool(
    ...
    async ({ title, details, approve_label, deny_label }) => {
      ...
      if (!daemon.available("approval")) return text(JSON.stringify({ approved: false, feedback: "Approvals are unavailable right now.", decided_by: "" }));
      try {
        const data = await daemon.call("approval", {
          approvalType: "agent",
          channelId,
          slug,
          authorId: createdBy,
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      if (!daemon.available("approval")) return decide(false, "Approvals are unavailable right now.");
      try {
        const data = await daemon.call("approval", { channelId, slug, authorId: createdBy, threadKey, toolName, toolInput: maskSecrets(toolInput) }, { timeoutMs: 280_000 });
