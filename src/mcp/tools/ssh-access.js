    ...
    async () => {
      ...
      if (setup.configured) {
        lines.push("", "Once granted, add this to `~/.ssh/config` on your laptop (your usual key; nothing per channel), then `ssh " + slug + "` or open it with VS Code Remote-SSH:", "```", connectSnippet({ endpoint: setup.endpoint, channel: slug }), "```",
          ...
          "Inside you are user `agent` in the channel's work folder with its `/home/agent`, CLI logins and Codex. `claude` there is this channel's Claude exactly as a message here gets it: the channel's tool policy, the gateway tools (no background jobs or approval cards — no thread to post into), your own Composio accounts as `composio-user`, the channel's as `composio-agent`, the channel's MCP servers and its secrets by name — signed in as the gateway's own account, whose usage it counts against. `codex` gets the same gateway tools, Composio accounts, channel MCP servers and secrets, and starts in the channel folder so its AGENTS.md applies; `with-secrets <command>` runs any other command with your channel secrets. Everyone in the box shares that one user; set your git identity per session. A daemon restart drops sessions — just reconnect.");
      }
