...
export function register(server, ctx) {
  ...
  server.registerTool(
    ...
    async ({ cron, in_minutes, run_at, prompt, description, notify, notify_user, kind, ack, ack_escalate_minutes, ack_dm_minutes }) => {
      ...
      if (oneTime) {
        ...
        const s = addSchedule({ channelId, slug, prompt, description, createdBy, notify: mode, notifyUserId, runAt: when.toISOString(), once: true, ...reminderFields });
      ...
      const s = addSchedule({ channelId, slug, cron, prompt, description, createdBy, notify: mode, notifyUserId, ...reminderFields });
      const what = reminderFields.kind === "reminder" ? "Reminder" : "Scheduled";
      return text(`✅ ${what} (id ${s.id}): "${description || prompt}" — cron \`${cron}\`, notifies ${who}${reminderFields.ack ? " · requires ✅" : ""}.`);
    }
  );
  ...
  server.registerTool(
    ...
    async () => {
      ...
      return text(
        list
          .map((s) => `• ${s.id} [${s.enabled ? "on" : "off"}] cron \`${s.cron}\` — ${s.description || s.prompt} (notifies ${s.notify === "user" ? `<@${s.notifyUserId}>` : s.notify || "channel"})`)
          .join("\n")
      );
    }
  );
