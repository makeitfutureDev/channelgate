...
// CG_THREAD_KEY is the RUN's session key, not necessarily a Slack thread_ts: a scheduled run uses
...
const currentThreadTs = () => slackThreadFor(process.env.CG_THREAD_KEY) || "";

export function register(server, ctx) {
  const { channelId, text } = ctx;
  ...
  server.registerTool(
    "slack_list_create",
    {
      description:
        ...
        "is_primary_column?, options?}). Returns the new list_id — save it to add/update items. NOTE: " +
        "the List is owned by the bot; share it to this channel in Slack so people can see it.",
      inputSchema: {
        name: z.string(),
    ...
    async ({ name, description, todo_mode, columns }) => {
      try {
        ...
        return text(`✅ Created List "${name}" — list_id \`${listId}\`.${cols ? `\nColumns: ${cols}` : ""}\nAdd rows now with slack_list_add_item / slack_list_update_item (this channel is already authorized). To make it VISIBLE to people, share the List into the channel in Slack.`);
      } catch (e) {
