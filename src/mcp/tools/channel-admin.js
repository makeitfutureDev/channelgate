...
export function register(server, ctx) {
  const { channelId, slug, createdBy, activeEngine, text, requireAdmin, requireManage, loadMeta, approvalSecret, approvalPort } = ctx;
      ...
      try {
        const started = startUpdate({
          ...
          context: {
            ...
            threadTs: slackThreadFor(process.env.CG_THREAD_KEY) || "",
            userId: createdBy,
          },
        });
        if (!started.ok) {
  ...
  server.registerTool(
    ...
    async () => {
      ...
      if (!secret) return text("Safe restart is unavailable right now.");
      try {
        const res = await fetch(`http://127.0.0.1:${port}/internal/restart`, {
          method: "POST",
