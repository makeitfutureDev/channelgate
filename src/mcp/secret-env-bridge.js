...
// The target must be a Node script this same runtime can exec — that restriction is what keeps
// this from becoming a general "run anything with a secret in its env" primitive. `.mjs`/`.cjs`
// count: the container image's gateway bridge is an ESM file in a directory with no package.json,
// where the extension is the only thing that declares the module type.
const [bundlePath, secretName, envName, scriptPath, ...scriptArgs] = process.argv.slice(2);
if (!bundlePath || !/^[A-Za-z][A-Za-z0-9]{0,80}$/.test(secretName || "") || !/^[A-Z][A-Z0-9_]{1,80}$/.test(envName || "") || !/\.(js|mjs|cjs)$/.test(scriptPath || "")) {
