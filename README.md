...
**The governed AI agent gateway for your Slack, Microsoft Teams and Google Chat channels — Claude Code and Codex, sandboxed per conversation, self-hosted.**
...
directly, or use `slack:CreateMessage` to post a visible Slack message that triggers Atlas. The
Slack path includes the verified module blueprint and live MakeItFuture IDs. Its key requirements
are a saved `<@U0BAQFWHNAK>` mention, an approved Make bot user, a trusted Make app/bot identity,
