...
- Full-access channels have their shell back. The admin-run settings file is the one an admin
  ...
  while the same turn spawned with the permission bypass and therefore had nowhere to send an
  approval to. Claude Code still evaluated the rule: a bare `pwd` came back denied, a command with
  a variable in it was refused outright, and the turn announced that the shell was not actually
  granted and continued read-only. Because the Full-access preset turns on admin mode alone (it
  never sets "allow shell"), that hit every channel using the preset as shipped. The admin variant
  now grants the shell tools outright and carries no ask rule, and it is generated by the same
  function as every other variant instead of being patched after the fact. Nothing widens: the file
  is only ever handed to an admin author in an admin-mode channel, whose turn already bypasses
  permission prompts. The shared file of the same channel is untouched, so a non-admin author there
  still gets an approval card for every command.
