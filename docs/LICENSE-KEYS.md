...
| `CHANNELGATE_PLATFORM_URL` | the platform base URL (default `https://makeitfuture.com/channelgate/api`); also settable in the UI |
| `CHANNELGATE_LICENSE_PAYLOAD` | a signed offline license (see below) |
| `CHANNELGATE_LICENSE_PUBLIC_KEY` | overrides the compiled-in verification key (staging only) |
