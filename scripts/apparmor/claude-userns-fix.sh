...
#
# Env:
#   CG_CLAUDE_ATTACH   override the profile's attachment glob (AppArmor syntax), e.g.
...
apply() {
    ...
    if [ -n "${CG_CLAUDE_ATTACH:-}" ]; then
        # Swap the attachment glob on the profile header line only.
        sed "s|^profile $PROFILE_NAME [^ ]* flags=|profile $PROFILE_NAME $CG_CLAUDE_ATTACH flags=|" "$PROFILE_SRC" > "$PROFILE_DST.tmp" \
            && install -m 0644 "$PROFILE_DST.tmp" "$PROFILE_DST" && rm -f "$PROFILE_DST.tmp"
        note "attachment overridden: $CG_CLAUDE_ATTACH"
    else
